...
for Hemi in ['lh', 'rh']:
	...
	print 'Generating the coorespondance map for hemisphere ' + Hemi
		...
		if (sp.call(['mri_label2label', '--srclabel', OutputDir+'/aparc-'+Hemi+'-%.3d_5thCol.label' %k,'--srcsubject', MovingSubject, '--trglabel', OutputDir+'/aparc-'+Hemi+'-%.3d_5thCol_Reg2MNI.label' %k,  '--trgsubject', ReferrenceSubject, '--regmethod', 'surface', '--hemi', Hemi,'--norevmap'])!=0):
			print ReD +'Transfering white-matter vertices to MNI space failed'+EndC 
			sys.exit(4)

		if (sp.call(['mri_label2label', '--srclabel', OutputDir+'/pial-aparc-'+Hemi+'-%.3d_5thCol.label' %k,'--srcsubject', MovingSubject, '--trglabel', OutputDir+'/pial-aparc-'+Hemi+'-%.3d_5thCol_Reg2MNI.label' %k,  '--trgsubject', ReferrenceSubject, '--regmethod', 'surface', '--trgsurf', 'pial', '--hemi', Hemi,'--norevmap'])!=0):
