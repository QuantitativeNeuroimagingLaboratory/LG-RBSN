...
function outputnum=Landmark_getRASlm_ds(subjectID,subjectdir,Outdir)
...
disp(['Generating Landmarks for Region-',name,'  ------ ~',num2str(100*noofregion/35),'%'])

outpath = [Outdir,'/region1',name,'/'];
disp(outpath)

Corrpoints = textread([path,'aparc-lh-',name,'_CorrespondanceMap2MNI_ds.label']);
...
Corrpoints2 = textread([path,'pial-aparc-lh-',name,'_CorrespondanceMap2MNI_ds.label']);
...
fid3 = fopen([outpath,'brain-lh-',name,'-RAS-ds-forview.label'],'wt');
...
fid4 = fopen([outpath,'brain-lh-',name,'-MNI-RAS-ds-forview.label'],'wt');
...
fid1 = fopen([outpath,'brain-lh-',name,'-VOX-ds.label'],'wt');
for ii = 1:outputnum
...
fid2 = fopen([outpath,'brain-lh-',name,'-MNI-VOX-ds.label'],'wt');
for ii = 1:outputnum
...
disp(['region1',name,'---landmarks number',num2str(outputnum)])
