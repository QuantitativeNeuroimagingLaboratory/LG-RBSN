...
function [grid1] = shootgrid_sumldmmom_vecintTPS(grid0,ldmdisp,lddmmoptions,varargin) % rhott displacement
...
function [W,points] = TPS(tt)
    ...
    rhot = [rhot;[16,16,240,0,0,0,16,16,16,0,0,0,16,240,16,0,0,0,16,240,...
        240,0,0,0,240,16,240,0,0,0,240,16,16,0,0,0,240,240,16,0,0,0,240,...
        240,240,0,0,0]'];
...
function dgrid = G_hd(tt,grid) % slooow version % dgrid is interpolated velocity vector fields -> yes derivative of displacement
    ...
    W = Wcell{find(tspan==t)};
    points = pointscell{find(tspan==t)};
    num = length(points);
...
siz = size(grid0{1});
% tspan = [0 1];
tspan = linspace(0,1,60);
Wcell = cell(length(tspan),1);
pointscell = cell(length(tspan),1);
for spant = 1:length(tspan)
    disp(['TPS time point [0,1] -- ',num2str(spant/6*10),'%'])
    [Wcell{spant} pointscell{spant}] = TPS(tspan(spant));
end
