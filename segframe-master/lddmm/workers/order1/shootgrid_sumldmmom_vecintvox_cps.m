...
function [grid1] = shootgrid_sumldmmom_vecintvox_cps(grid0,ldmdisp,lddmmoptions,varargin) % rhott displacement
...
%     [Kor]=radialBasis(distance);
    ...
    disp(['TPS dimension - 1'])
    w1 = Y(:,1)'/Lmat;
    
    pause(3)
    disp(['TPS dimension - 2'])
    w2 = Y(:,2)'/Lmat;
    
    pause(3)
    disp(['TPS dimension - 3'])
    w3 = Y(:,3)'/Lmat;
    clear Lmat; clear P; clear vec; clear Y
    W = [w1;w2;w3]';
    clear w1; clear w2; clear w3
       
    disp(['Apply Weight to Grid'])
%        Ngriddur = length(grid);
       ...
       [tran_K]=radialBasis_cps(sqrt(tran_K));
%        tran_K = sqrt(tran_K);
