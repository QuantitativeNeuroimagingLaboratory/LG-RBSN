...
addpath('/share/projects/razlighi_lab/users/hengda/RDIR_LM/segframe-master/thirdparty/spm')

SUB = 'P00004368';
hm = 'lh';

projectdir='/share/projects/razlighi_lab/users/hengda/RDIR_LM';
imagedir = ['/share/projects/razlighi_lab/users/hengda/RDIR_LM/Workspace_',SUB];
...
%    
% for sizeofkernel = [4:6]
% 
%     disp(['KernelSize = ',num2str(sizeofkernel)])
...
submaskmap = cell(length(regioninput),1);
   for i = 1:length(regioninput)
       ...
       subwindow = suball((boundingbox(i,3)-2 - trans(i,2)):(boundingbox(i,4)+2- trans(i,2))...
          ,(boundingbox(i,1)-2- trans(i,1)):(boundingbox(i,2)+2- trans(i,1)),...
          (boundingbox(i,5)-2- trans(i,3)):(boundingbox(i,6)+2- trans(i,3)));
...
%         % original
%             [xse,yse,zse] = ndgrid(-3:3);
%             se = strel(sqrt(xse.^2 + yse.^2 + zse.^2) <=sizeofkernel); 
%             se.Neighborhood
%             [xse,yse,zse] = ndgrid(-2:2);
%             se = strel(sqrt(xse.^2 + yse.^2 + zse.^2) <=sizeofkernel); % DICE 86.78
% %             
              [xse,yse,zse] = ndgrid(-2:2);
            se = strel(sqrt(xse.^2 + yse.^2 + zse.^2) <=4); % DICE 86.78          
%             
%                           [xse,yse,zse] = ndgrid(-2:2);
%             se = strel(sqrt(xse.^2 + yse.^2 + zse.^2) <=2); % DICE 86.78    
            imgsize = size(submask);

            chosensubmask = imdilate(submask,se);
%             chosensubmask = ones(imgsize);
%           chosensubmask = submaskmap{i};  % bx          
            
            
            subwindow(find(subwindow==regioninput(i))) = 0;
            subwindow(find(subwindow~=0)) = 1;
            chosensubmask = chosensubmask - subwindow; % subtract other regions
            chosensubmask(find(chosensubmask~=1)) = 0;
...
%            displacementregion{i,d} = displacement{i,d};
           ...
           % set up mask
            displacementregionmask{i,d} = chosensubmask;
...
% simple mean linear transformation
...
for d = 1:3
    ...
    V = cell(3,1);
for d = 1:3
    V{d} = zeros(length(gridy),1);
    fieldint = fieldall{d}(bx(3):bx(4),bx(1):bx(2),bx(5):bx(6));
    for t = 1:length(gridy)
        V{d}(t) = fieldint(gridy(t),gridx(t),gridz(t));
    end
end
% object = zeros(prod(size(fieldint)),3);
% for d = 1:3
%    object(:,d) = reshape(gridFixedimage{d},prod(size(fieldint)),1); 
% end

Vq = cell(3,1);
[yq,xq,zq] = meshgrid(1:size(fieldint,1),1:size(fieldint,2),1:size(fieldint,3));
yq = permute(yq,[2 1 3]);
xq = permute(xq,[2 1 3]);
zq = permute(zq,[2 1 3]);
% Xq = cell(3,1);
...
% Zq = cell(3,1);
for d = 1:3
    Vq{d} = griddata(gridx,gridy,gridz,V{d},xq,yq,zq,'linear');
end

for d = 1:3
    Vq{d}(find(isnan(Vq{d}))) = 0;
end
fieldint = Vq{1};
[gridy,gridx,gridz] = ind2sub(size(fieldint),find(fieldint ~= 0));
...
for d = 1:3
    Vq{d} = griddata(gridx,gridy,gridz,V{d},xq,yq,zq,'nearest');
end
% for d = 1:3
%     Vq{d}(find(isnan(Vq{d}))) = meantrans(1,d);
% end
...
for d = 1:3
    gridMovingimage{d} = gridFixedimage{d} + Vq{d};
end    


% [x,y,z] = meshgrid(gridx,gridy,gridz);
% for d = 1:3
% %     Vq{d} = interp3(gridx,gridy,gridz,V{d},object(:,2),object(:,1),object(:,3));
%     
%     F = scatteredInterpolant(gridx,gridy,gridz,V{d});
%     Vq{d} = F(x,y,z);
% 
% end
...
% end
% display('Running TPS')
% tps = TPS3D(source,target,object);
% 
%
