...
function Apply2region_withaffinedisp(projdir,SUB,regioninput,kernelsize,weight)
%regioninput=1032;
...
disp([datadir,'trans-',region(2:end),'.txt'])
trans = textread([datadir,'trans-',region(2:end),'.txt']);


disp([datadir,'aparc+aseg_',region,'.nii'])
unix(['gzip -d ',datadir,'aparc+aseg_',region,'.nii.gz']);
[sub subheader] = rest_ReadNiftiImage([datadir,'aparc+aseg_',region,'.nii']);
unix(['gzip ',datadir,'aparc+aseg_',region,'.nii']);

%% affine
affine = load([projdir,'/Subject/',SUB,'/displacement.mat']);
affine = affine.displacement;
%% lddmm
